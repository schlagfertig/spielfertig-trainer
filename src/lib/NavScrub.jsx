...
export function NavScrub({ items, index, disabled, onPick }) {
  ...
  function previewAt(i) {
    const row = list[clamp(i, 0, list.length - 1)];
    return row || null;
  }
  ...
  function onTouchMove(e) {
    ...
    if (st.mode === "swipe") {
      const steps = Math.round(-dx / 42);
      const nextI = clamp(st.start + steps, 0, list.length - 1);
      setHover(nextI);
      setWheel(true);
    }
    if (st.mode === "wheel") {
      const steps = Math.round(-dx / 28);
      setHover(clamp(st.start + steps, 0, list.length - 1));
    }
  }
    ...
    >
      {wheel && shown ? (
        <div style={{
          ...
          <div style={{ textAlign: "center", color: "#f4f7f6", font: "700 22px/1.15 Oswald, sans-serif", marginTop: 4 }}>
            {shown.preview || shown.label}
          </div>
          <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 10, overflow: "hidden" }}>
            {list.map((row, i) => {
