...
export function RudimentStaff({ rud, playingT = -1, handwritten, svgId }) {
  ...
  return (
    <svg id={svgId} viewBox={`0 0 ${w} 140`} width="100%" role="img" aria-label={rud.label}>
      <g stroke={INK} fill="none" strokeLinecap="butt" strokeLinejoin="miter">
        ...
        {notes.map((nt, i) => {
          ...
          return (
            <g key={i}>
              ...
              {nt.hand && (
                <text x={x} y={y + 2 * lineGap + 18} textAnchor="middle" fontSize="12" fontWeight="700" fill={nt.hand === "R" ? RCOL : LCOL} stroke="none">
                  {nt.hand}
                </text>
              )}
            </g>
        ...
        {groups.map((g, gi) => {
          ...
          const y0 = stemTop;
          const sl = Math.min(2.2, (xs[xs.length - 1] - xs[0]) * 0.012);
          ...
          for (let b = 0; b < maxB; b++) {
            g.forEach((n, i) => {
              ...
              if (i < g.length - 1 && beamsFor(g[i + 1]) > b) {
                ...
                const hook = Math.min(Math.abs(neighborX - xs[i]) * 0.48, stepW * 0.7);
                layers.push(
